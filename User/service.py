...
from User.schemas import UserCreate, Status
import shutil
...
class UserService:
    ...
    async def get_users_by_jobid_status(job_id: int, status: Optional[str], db: AsyncSession):
        query = select(UserModel.User,
                       ...
                       VideoProcessing.processing_status,
                       VideoProcessing.result_quality,
                       VideoProcessing.queued_at,
                       VideoProcessing.started_at,
                       VideoProcessing.completed_at,
        ...
        quality_priority = case(
            (VideoProcessing.result_quality == "complete", 0),
            (VideoProcessing.result_quality == "partial", 1),
            (VideoProcessing.result_quality == "failed", 2),
            else_=3,
        )

        query = query.order_by(
            quality_priority.asc(),
            desc(VideoProcessing.total_score).nulls_last(),
        )
        ...
        if status == Status.PENDING.value:
            ...
            for user, total_score, processing_status, result_quality, queued_at, started_at, completed_at in rows:
                user_dict = {
                    "userId": user.id,
